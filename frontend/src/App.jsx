...
function Inquiry({onOrderCreated,userName}){
  ...
  const process=async()=>{setLd(true);setRes(null);setErr("");const custCode=makeId(cust||"Ügyfél","");try{const text=await ai([{role:"user",content:`Process this customer inquiry for a Hungarian auto parts business. Respond ONLY with valid JSON:\n\nCustomer: ${cust||"Unknown"} (code: ${custCode})\nMessage: "${msg}"\n\n{"parts":[{"name":"Hungarian part name (specific, e.g. Fékbetét első tengely)","qty":1,"allegroUrl":"https://allegro.pl/listing?string=exact+polish+search+terms+with+car+model","estimatedPricePLN":0}],"car":"FULL spec: make + model + generation code + year e.g. Mercedes-Benz C-Class W204 2008, BMW 3 Series E90 2007, Opel Astra J 2012 — NEVER just brand alone — if year or model missing set to null and ask in reply","totalEstimatePLN":0,"totalEstimateHUF":0,"priceNote":"brief note on price range and factors e.g. OEM vs aftermarket","sellerMessagePL":"Polish message starting with customer code listing ALL parts with quantities asking about availability and price — Krakow shipping","customerReplyHU":"Hungarian reply listing all parts with rough price estimate in HUF — if car model or year is missing ask the customer to specify"}`}]);setRes(JSON.parse(text.split(String.fromCharCode(96,96,96)+"json").join("").split(String.fromCharCode(96,96,96)).join("").trim()));}catch{setErr("Hiba. Próbálja újra.");}setLd(false);};
