...
function CatalogueManager({user}){
  const[items,setItems]=useState([]);const[learned,setLearned]=useState({}); // {serial: {car,partName,confirmedAt}}const[showForm,setShowForm]=useState(false);const[images,setImages]=useState([]);const[analyzing,setAnalyzing]=useState(false);const[form,setForm]=useState(BF);const[saving,setSaving]=useState(false);
  ...
  const f=(k)=>({value:form[k],onChange:v=>{
    setForm(x=>({...x,[k]:v}));
  ...
  useEffect(()=>{
    db.get("catalogue_items",true).then(d=>setItems(d||[]));
    db.get("parts_learned",true).then(d=>setLearned(d||{}));
  }, []);
  const handleFiles=async(files)=>{const b64s=await Promise.all(Array.from(files).map(file=>new Promise(res=>{const r=new FileReader();r.onload=()=>res(r.result);r.readAsDataURL(file);})));setImages(prev=>[...prev,...b64s]);setAnalyzing(true);const knownSerials=Object.entries(learned).slice(0,20).map(([s,v])=>`${s} = ${v.car} (${v.partName})`).join("\n");
    try{const content=[...b64s.map(b64=>({type:"image",source:{type:"base64",media_type:b64.split(";")[0].split(":")[1],data:b64.split(",")[1]}})),{type:"text",text:`You are an expert auto parts identification specialist. Analyze these car part images carefully.\n\nCRITICAL SERIAL NUMBER RULES:\n- Examine every number/letter stamped, engraved, or printed on the part\n- Read digit by digit — do NOT guess or auto-correct\n- Common misreads: 0 vs O, 1 vs I vs l, 5 vs 6, 6 vs 5, 8 vs B, 2 vs Z\n- After reading, cross-reference the serial to verify it matches the part you identified\n- Flag any ambiguous characters in serialNumberWarning\n\nCAR IDENTIFICATION:\n- ONLY name a car if you can SEE it written on the part (sticker, embossed, casting). NEVER guess from serial number alone.\n- If not visible on part set car to null - write Nem lathato a kepen.\n- Use known corrections if serial matches: ${knownSerials}\n\nRespond ONLY with valid JSON:\n{"partName":"specific Hungarian part name e.g. Első féktárcsa — include make/model if visible on part","serialNumber":"every digit copied exactly as seen on part — null if not visible","serialNumberVerified":"what this serial corresponds to — confirms or corrects the part name","serialNumberWarning":"flag any ambiguous characters e.g. position 3 is 5 or 6 — null if clean","serialNumberConfidence":"high/medium/low","car":"compatible vehicles e.g. VW Golf VII 2013-2020 — null if unknown","condition":"Jó","estimatedPricePLN":0,"estimatedPriceHUF":0,"priceNote":"OEM vs aftermarket price range","description":"2-3 sentence professional Hungarian description of part, condition, and fitment"}`}];const text=await ai([{role:"user",content}]);const d=JSON.parse(text.split(String.fromCharCode(96,96,96)+"json").join("").split(String.fromCharCode(96,96,96)).join("").trim());setForm(x=>({...x,partName:d.partName||"",serialNumber:d.serialNumber||"",serialNumberVerified:d.serialNumberVerified||"",serialNumberWarning:d.serialNumberWarning||"",serialNumberConfidence:d.serialNumberConfidence||"",car:d.car||"",condition:d.condition||"Jó",description:d.description||"",estimatedPriceHUF:d.estimatedPriceHUF||"",priceNote:d.priceNote||""}));}catch{}setAnalyzing(false);};
  const publish=async()=>{if(!form.partName||!form.price)return;setSaving(true);
    if(form.serialNumber&&form.car) await saveLearn(form.serialNumber,form.car,form.partName);
    const item={id:Date.now(),...form,images,publishedBy:user.name,publishedAt:new Date().toISOString(),sold:false};const updated=[item,...items];await db.set("catalogue_items",updated,true);setItems(updated);setShowForm(false);setImages([]);setForm(BF);setSaving(false);};
  ...
  return(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24}}><PH sub="Alkatrészek közzététele a nyilvános katalógusban">Katalógus</PH><Btn onClick={()=>{setShowForm(!showForm);setImages([]);setForm(BF);}}>{showForm?"✕ Mégse":"+ Új alkatrész"}</Btn></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}><Field label="Alkatrész neve" {...f("partName")} placeholder="pl. Féktárcsa első"/><Field label="Sorozatszám / OEM" {...f("serialNumber")} placeholder="pl. 1K0615301AA"/>{form.serialNumber&&learned[form.serialNumber.toUpperCase()]&&<div style={{gridColumn:"1/-1",background:C.green+"08",border:`1px solid ${C.green}20`,borderRadius:6,padding:"8px 12px",fontSize:11,color:C.green,display:"flex",alignItems:"center",gap:6}}><span>🧠</span><span>Tanult: <strong>{learned[form.serialNumber.toUpperCase()].car}</strong> · {learned[form.serialNumber.toUpperCase()].partName}</span><button onClick={()=>setForm(x=>({...x,car:learned[form.serialNumber.toUpperCase()].car,partName:learned[form.serialNumber.toUpperCase()].partName}))} style={{marginLeft:"auto",background:C.green+"20",color:C.green,border:"none",borderRadius:4,padding:"2px 8px",fontSize:10,cursor:"pointer",fontFamily:F}}>Alkalmaz</button></div>}{form.serialNumberWarning&&<div style={{gridColumn:"1/-1",background:C.amber+"10",border:`1px solid ${C.amber}25`,borderRadius:6,padding:"8px 12px",fontSize:11,color:C.amber}}>⚠ Bizonytalan karakter: {form.serialNumberWarning}</div>}{form.serialNumberVerified&&<div style={{gridColumn:"1/-1",background:C.green+"08",border:`1px solid ${C.green}20`,borderRadius:6,padding:"8px 12px",fontSize:11,color:C.t2}}>✓ Azonosítás: {form.serialNumberVerified}</div>}<Field label="Kompatibilis jármű" {...f("car")} placeholder="pl. VW Golf VII 2013-2020"/><Field label="Állapot"><select value={form.condition} onChange={e=>setForm(x=>({...x,condition:e.target.value}))} style={inp}>{CONDITIONS.map(c=><option key={c}>{c}</option>)}</select></Field><Field label="Ár (Ft)" {...f("price")} placeholder="pl. 25000"/>{form.estimatedPriceHUF&&!form.price&&<div style={{gridColumn:"1/-1",background:C.blue+"08",border:`1px solid ${C.blue}20`,borderRadius:6,padding:"8px 12px",fontSize:11,color:C.t2}}>💡 AI árbecslés: ~{Number(form.estimatedPriceHUF).toLocaleString("hu")} Ft{form.priceNote?" — "+form.priceNote:""} <button onClick={()=>setForm(x=>({...x,price:String(form.estimatedPriceHUF)}))} style={{marginLeft:8,background:C.blue+"20",color:C.blue,border:"none",borderRadius:4,padding:"2px 8px",fontSize:10,cursor:"pointer",fontFamily:F}}>Használ</button></div>}<Field label="Elérhetőség" {...f("contact")} placeholder="pl. +36 30 123 4567"/><Field label="Átvételi hely" {...f("pickup")} placeholder="pl. Budapest XV."/></div>
